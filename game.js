...
function start() {
    ...
    speed = 3.8; sloMo = 1; overlay.style.display = 'none';
    lastTime = performance.now();
    requestAnimationFrame(loop);
}
function spawn() {
    // gap shrinks more slowly than before, min gap increased a bit
    let gap = Math.max(120, 220 - (score * 0.9));
    let center = Math.random() * (H - gap - 160) + 80 + gap/2;
    // vertical move grows slower and has a lower cap
    let move = score >= 30 ? Math.min(80, (score-30)*2) : 0;
...
function loop(t) {
    ...
    // מהירות גדלה בקצב איטי יותר מאשר קודם
    speed = 3.8 + (score * 0.045);
