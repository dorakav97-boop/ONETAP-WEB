window.addEventListener('load', () => {
    ...
    let gravity = 0.35, jump = -6.5;
    
    const stageConfigs = [
        { bg: "#05080a", pipe: "#06b6d4" }, // שלב 1
        { bg: "#1a0b2e", pipe: "#d946ef" }, // שלב 2
        { bg: "#062c43", pipe: "#0ea5e9" }, // שלב 3
        { bg: "#431407", pipe: "#f97316" }, // שלב 4 - זזים!
        { bg: "#020617", pipe: "#ffffff" }  // שלב 5
    ];
    ...
    function loop(t) {
        ...
        for (let i = pipes.length - 1; i >= 0; i--) {
            ...
            if (!p.passed && p.x < player.x) {
                p.passed = true; score++; 
                currentSpeed += 0.05; // המהירות עולה משמעותית
        ...
        for (let i = items.length - 1; i >= 0; i--) {
            ...
            if (Math.hypot(player.x - it.x, player.y - it.y) < player.r + it.r) {
                ...
                currentSpeed += 0.10; // מטבע מאיץ חזק
    ...
    window.addEventListener('touchstart', (e) => { if (running && e.target.tagName !== 'BUTTON') player.vy = jump; }, {passive: false});
